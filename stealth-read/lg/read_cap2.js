    ...
    if(step&&sqlp&&esqlp){clearInterval(st);
      ...
      Interceptor.attach(step,{onEnter:function(a){
        try{var p=sqlf(a[0]);if(p.isNull())return;var s=p.readCString();if(!s)return;
          var isW=(s.indexOf('UPDATE')===0||s.indexOf('REPLACE')===0||s.indexOf('INSERT')===0);
          if(!isW)return;
          if(s.indexOf('me_read')>=0||s.indexOf('read_position')>=0||s.indexOf('message_read')>=0||s.indexOf('read_time')>=0||s.indexOf('read_state')>=0){
            var es=''; try{var ep=esqlf(a[0]); if(!ep.isNull()) es=ep.readCString();}catch(e){}
            send({ev:'WRITE', tid:this.threadId, sql:(es||s).substring(0,360)});
          }
        }catch(e){}
      }});
      send({ev:'read_cap2_armed'});
    }
  },200);
});
})();
