...
for i in $(seq 1 15); do
  ...
  /tmp/frida1762/bin/python attach_drive.py hunt22.js >/tmp/h22hold.log 2>&1 &
  P=$!
  sleep 11
  if grep -q content_drop_armed /tmp/h22hold.log; then
    echo "ARMED on attempt $i (pid $P) — holding"
    wait $P
    exit 0
  fi
  echo "attempt $i failed: $(tail -1 /tmp/h22hold.log | grep -oE 'TransportError.*|TimedOut.*|ServerNot.*' | cut -c1-40)"
  kill -9 $P 2>/dev/null
